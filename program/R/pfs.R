...
describe(KM1, "PFS_KM1_survival")
describe(KM2, "PFS_KM2_survival")
describe(KM3, "PFS_KM3_survival")

ptdata$event2 <- with(ptdata,
                      ifelse(SUDTHFL == "死亡", "死亡",
                             ifelse(RECURRYN == "あり", "増悪・再発",
                                    NA)))
dfCount <- ptdata %>%
  select("event2", "group") %>%
  table %>%
  as.data.frame.matrix
write.csv(dfCount, paste0(outpath, "/PFS_event.csv"), row.names = TRUE, na = "")
